...
console.log(data);
          try {
            const res = await axios.post("http://localhost:5000/api/employee/signup", {
              name: data.name, 
              email: data.email,
    ...
    return (
      ...
      <div className="relative flex flex-col justify-center h-screen overflow-hidden">
          ...
          <label
            ...
            {errors.password && errors.password.type === "pattern" && (
              <p className="errorMsg">
                Password should be 8 characters length, 2 letters in Upper Case, 1 Special Character (!@#$&*), atleast 2 numbers,3 letters in Lower Case.
              </p>
            )}
          </div>
          <div className="my-5 w-full">
      ...
      <button type="primary" className='group relative w-full flex justify-center py-2 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 mt-10'>Signup</button>
        </form>  ) : ('their is no result here')}
</div>
</div>
</div>
    );
